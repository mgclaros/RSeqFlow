...
Let's see how many genes are not expressed in your samples, expressed in all samples and expressed in some samples:
...
> **IMPORTANT:** The filtering cannot rely on `counts` when library sizes are very different. Hence, [CPM] (_counts per million_) should be used instead of counts. This is why the value of the `min.count` argument in `filterByExpr()` is  converted into cpm before using it as a threshold.
