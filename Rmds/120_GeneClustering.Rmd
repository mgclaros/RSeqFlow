...
2. A method **based on 30 metrics** can be obtained using `NbClust()` function from the `NbClust` package. Using the parameter `index = "all"` is very quick but uses only 26 metrics, excluding GAP. The complete set is more time-consuming and requires to change the function call with  with `index = "alllong"`.
...
```{r orientativePlots, fig.width=5, fig.height=6, out.width=c('33%', '33%', '33%'), fig.show='hold'}
# scree plot
...
The **optimal cluster number** `k_30` can be calculated with a longer method based on 26 (`index = "all"`) metrics to be sure that the selected value is supported by most methods, not only one. Severla plots and messages are generated during the evaluation.
...
```{r gene-silueta, fig.height=10, fig.width=4, eval=FALSE}
...
m.putat.std.clust <- cbind(as.data.frame(m.ctf.std), Cluster = groups_k)
