...
* after the correlations between the columns (factors, conditions, samples) using `cor()`, as [calculated above with raw data](#raw-sample-correlations); correlations are then converted to distances using `as.dist()` function.
...
2. Let's use the best linkage method in `hclust()` function to produce a `hclust` object; these variables start by `HC_??`.
    ```{r samples-HCA}
    # for CTF
...
The `hclust` object can be plotted as a dendrogram to check whether the different branches correspond to sample replicates.

```{r sample-dendro, fig.width=6, fig.height=4, out.width=c('50%', '50%'), fig.show='hold'}
plot(HC_samples_ctf,                             # CTF dendrogram
     main = paste0("CTF.std - d:", CORR_METHOD, " — HCA: ", best_meth_ctf))
rect.hclust(HC_samples_ctf,                      # draw rectangles around clusters
...
plot(HC_samples_tmm,                             # TMM dendrogram
     main = paste0("TMM.std - d:", CORR_METHOD, " — HCA: ", best_meth_tmm))
rect.hclust(HC_samples_tmm, 
...
The **silhouette plot** can help to determine how well each observation (i.e. sample) lies within its dendrogram branch: a high average silhouette width indicates a good contribution. The average silhouette is the mean of observations (samples) for different values of experimental factors defined by the user in `EXP_FACTORS`.
...
plot(silhouette(cutree(HC_samples_ctf, k_samples), 
                d_ctf_samp), 
     main = "CTF")
plot(silhouette(cutree(HC_samples_tmm, k_samples), 
                d_tmm_samp), 
     main = "TMM")
```

`r NOTE_bx` 
If there is a sample with negative contribution, you may consider to remove it and reanalyse the complete dataset.
...
`r NOTE_bx` 
Correlations based on normalised [DEG]s should group samples better than un-normalised highly variant genes.
</div>
