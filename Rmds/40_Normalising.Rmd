...
The number of pincipal components in [PCA] that merit consideration are those whose `eigenvalues > 1` (where an eigenvalue is the standard deviation of such component) since they are components actually explaining more than a single explanatory variable. This can be inspected in the following scree plot:


```{r PCAcompoments, results='hold', fig.width=8, fig.height=6, eval=FALSE, echo=FALSE}
numPCs <- 10  # number of PCs to be plotted
# el tmp tiene que ser el presultado de prcomp() y no el summary
screeplot(tmp, 
          type = "l", 
          npcs = 1:numPCs,  
          main = paste0("Screeplot of the first ", numPCs, " PCs")
)
