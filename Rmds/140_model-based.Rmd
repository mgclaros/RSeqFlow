...
if (fit$G == 1) {
  message("The **Mclust** algorithm found only one cluster. Then k-means clusters will be assigned")
  groups_mclust <- groups_kmeans
  mclust_k <- as.integer(max(names(table(groups_mclust))))
} else {
  message("The **Mclust** algorithm found **", fit$G, "** clusters")
  mclust_k <- fit$G
}
```
