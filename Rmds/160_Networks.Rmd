...
`r NOTE_bx`
Only clusters **including >2 genes** will be analysed. The rest will be removed.
</div>
...
i <- 1
for (i in 1:final_k) {
  hasMoreThan1Row <- !is.null(nrow(m.ctf.std[groups_k == i, ]))
  if (hasMoreThan1Row) {
    id <- paste0(txt, i)
...
i <- 1
for (i in 1:kmeans_k) {
  hasMoreThan1Row <- !is.null(nrow(m.ctf.std[groups_kmeans == i, ]))
  if (hasMoreThan1Row) {
    id <- paste0(txt, i)
...
i <- 1
for (i in 1:mclust_k) {
  hasMoreThan1Row <- !is.null(nrow(m.ctf.std[groups_mclust == i, ]))
  if (hasMoreThan1Row) {
    id <- paste0(txt, i)
...
`r OK_bx`
The clusters to be analysed are: 

`r toString(names(CLUSTER_LIST))`.
</div>
...
`r NOTE_bx`
To construct networks, only clusters **including significant correlations** will be analysed. The rest will be removed.
</div>
...
# remove clusters from CLUSTER_LIST that do not contain significant correlations
...
`r OK_bx`
The clusters that will produce networks are: 
...
```{r Allcommunities, results='asis', out.width='50%'}
i <- 1
for (i in 1:list_length) {
  if (ncol(CLUSTER_LIST[[i]]$expr_matrix) < 2) next # avoid representation of one-membered clusters
  # tab header
  ...
  plot(aNet,
       ...
       vertex.label.font = 2,             # 1 plain, 2 bold, 3 italic, 4 bold italic, 5 symbol
       vertex.label.family = "sans",      # font family: serif, sans, mono
       vertex.size = 3,                   # tamaño del nodo
       vertex.color="tomato",             # color del nodo
       vertex.label.cex = 0.9,            # tamaño de la etiqueta
       vertex.label.color = "darkviolet", # color de la etiqueta
       vertex.label.dist = 1.5,           # distancia de la etiqueta al nodo
       edge.arrow.size = 0.3,             # intenta cambiar el tamaño del conector
       mark.groups = ceb,                 # colorea las comunidades
       margin = -0.01,                    # aumenta el área de impresión; 0.05 ya deja cosas fuera
...
`r NOTE_bx`
The list of edges and their parameters contained in the `corr_df` matrix for each cluster and saved in the `List_of_clusters-{...}.Rds` can be read in R using `r newList <- readRDS(file = fileName)`. 
</div>

The `BestCorrelations_{...}.tsv` files are lists of edges for every cluster. **They can be loaded into [Cytoscape](http://www.cytoscape.org/) for further analysis** as described in [@RodriGutierrez2018]:

1. Launch Cytoscape
2. Import the network table `BestCorrelations_{...}.tsv`: ***File > Import > Network > File***
3. Indicate where relevant information is stored in the file: 
  - Source interaction is ***Item1***
  - Target interaction is ***Item2***
4. After the network has been displayed, use `organic layout` which can be found under the ***Layout > Files Layout > Organic*** menu to facilitate identification of highly connected nodes
...
6. In the pop-up window, the drop-down list `Network Collection` select the imported network `BestCorrelations_{...}.tsv`
...
10. On the new menu displayed, click on ***Column*** to show a new menu in which select ***Rank_- stat*** as the attribute to determine the size. ***Mapping type*** should be in ***Continuous Mapping*** option to distribute the sizes continuously along the previously generated combined ranking for each node.
