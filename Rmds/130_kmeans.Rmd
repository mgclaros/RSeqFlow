...
```{r kmeans-of-30, warning=VERBOSE_MODE, results='hide'}
# bext-of-30 for kmeans
nb <- NbClust(m.ctf.std, 
              distance = "euclidean", 
              min.nc = MIN_NC,   # default values defined in chunk k-30
              max.nc = MAX_NC,
              index = "all",
              method = "kmeans")
...
### Final number of k-means clusters
...
Let's exploit the `kmeans()` function to assign genes to clusters.
...
Let's see the detailed number of genes per cluster .
...
```{r TODO, echo=FALSE}
# Since we know that the genes should be distributed by the experimental factors, does the _k_-means clustering matches with the true structure of the data? 
