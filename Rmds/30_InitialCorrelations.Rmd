...
* **Pearson**'s _r_ is the most popular correlation measure, although it assumes a linear correlation and normally distributed values, but it is sensitive to outliers. It measures the correlation between a pair of expression profiles.
* **Spearman**'s $\rho$ rank correlation is more robust (less sensitive to outliers), but also less powerful than _r_ because it distributes values in ranges and looks for linear correlation between ranks. Useful when you have at least one qualitative variable.
* **Kendall**'s $\tau$ is less powerful, but useful when all values are qualitative.
* **Mutual Information** (MI) attempts to measure sthe tatistical dependence between two variables

</details> 
[END EXPANDIBLE]: #

> **IMPORTANT:** Since all variables are quantitative, `pearson` method can be used, or you may prefer `spearman` to correlate tendencies (instead of linearity). 
