...
1. Log-transformation of expression values.
...
## Sample correlations {#raw-sample-correlations}
...
`r INFER_bx` 
Even though all variables are quantitative, it is preferable to use `method = 'spearman'` to [correlate tendencies (instead of linearity)](https://medium.com/analytics-vidhya/covariance-vs-pearson-correlation-coefficient-vs-spearmans-rank-correlation-coefficient-1d2d8789a728). 
</div>
...
`r NOTE_bx` 
  Since this step is only exploratory, no _P_ value will be calcuated to speed up the analysis.
  
  Remember that you indicated in the configuration file the use of `r CORR_METHOD` method to calculate correlations.
</div>
...
2. Plotting correlations using two different methods. The left method includes the correlation values and the right plot uses the circle size to represent the correlation value. In both cases, the color intensity is related to the correlation value and sign.

```{r corPlotting, out.width="50%", fig.show='hold'}
# corPlot is defined in library psych
...
# corrplot comes from library corrplot
corrplot(corr_samples_r)
```

`r NOTE_bx` 
If samples are correctly grouped, data are very coherent. However, even if samples apparently produce odd groups, this might be corrected after normalisation and differential expression (see next sections)
</div>


## Dendrogram of samples {#grouping-raw-samples}

A good way to see if raw samples behave as expected respect to replicates and experimental conditions is to group them by similarity and then plot the result as a **dendrogram**. [Fundamentals of clustering will be given below](#clustering) and also in <https://www.statology.org/hierarchical-clustering-in-r/>. You can also inspect some [clustering code](https://github.com/Statology/R-Guides/blob/main/hierarchical_clustering.R).

Two dendrograms will be calculated with the function `stats::hclust()` using two agglomeration methods:

1. Using the default method (`method = "complete"`).
2. Using the ward.D2 method (`method = "ward.D2"`), where the dissimilarities are squared to minimise the total. This method is usually more appropriate for RNA-seq data.

Dendrograms will be plotted cutting the tree by the number of unique factor described in the experiment (`length(UNIQ_FACTORS)`) to facilitate the interpretation.

```{r dendrograms, out.width="50%", fig.show='hold'}
hcols <- hclust(as.dist(1 - corr_samples_r), 
                method = "complete") 
# 'complete' finds the max distance between points belonging to two different clusters.
plot(hcols,
     main = "Dendrogram of sample clusters using 'complete'")
rect.hclust(hcols,                        # draw rectangles around clusters
...
hcols <- hclust(as.dist(1 - corr_samples_r), 
                method = "ward.D2") 
plot(hcols,
      main = "Dendrogram of sample clusters using 'ward.D2'")
rect.hclust(hcols,
            k = length(UNIQ_FACTORS),
            border = unique(EXP_COLORS))
```

## Heatmap of samples

A heat map is a false color image with a dendrogram added to rows and/or columns. Reordering at least rows (genes) is recommended to produce meaning heatmaps. Sample dendrogram was calculated before (`hcols` variable), and the dendrogram of rows is now calculated based on correlations. Main gene clusters will be calculated and coloured for the heatmap.

```{r geneclusters}
# clustering by gene
hrow <- hclust(as.dist(1-cor(t(matriz_filt), 
                             method = CORR_METHOD)), 
               method = "complete")

mycl <- cutree(hrow, h = max(hrow$height)/1.5)  # Obtain discrete clusters
mycolhc <- sample(rainbow(256))                 # Give colours to clusters
mycolhc <- mycolhc[as.vector(mycl)]             # assing color cluster to genes
```


Now we can use both hierarchical clusters to construct a heatmap, but showing only the dendrogram of samples sin the one of genes is very crowded and difficult to interpret.

```{r heatmap}
heatmap.2(matriz_filt, 
          dendrogram = "column",           # no dengrogran printed for row/genes
          Rowv = as.dendrogram(hrow),      # precalcuated dendrogram for rows
          Colv = as.dendrogram(hcols),     # precalcuated dendrogram for columns
          col = HEATMAP_COLORS,            # colour palette for expression levels
          labRow = FALSE,                  # no row (gene) labels
          scale = "row",                   # scale rows
          trace = "none",                  # no expression trace
          key = TRUE,                      # show colour scale
          key.title = "Colour scale",      # title of colour scale
          ColSideColors = EXP_COLORS,      # give colors to colums by experimental factor
          RowSideColors = mycolhc)         # give colors to genes by cluster
```

`r NOTE_bx` 
Samples should be grouped by replicates. 
</div>
