...
1. If there is any **`ubi`** DEG, save it, since they can be good **biomarker candidates**, although their utility for clustering, networking functional analyses is less significant.
    ```{r DEGs-in-all, results='hold'}
    ...
    cat("Number of ubiquitous genes after treat(): ", ubis.treat,
        "\nNumber of ubiquitous genes after eBayes(): ", ubis.eB, "\n")
    
    # Save ubiquitous DEGs for further utility
    if (ubis.eB >= 1) {
      fileName <- SaveTSV(ubi.DEGs, "ubiquitousDEGs_")
      message("IDs of ubiquitous DEGs have been saved in file", "\n", fileName)
      if (ubis.treat > 0) {
        cat("\n -- Main BIOMARKER CANDIDATES -- \n", toString(ubis.treat.IDs))
      }
    }
    
    # keep ubi IDs as the first outstanding genes
    OUTSTANDING_GENE_LIST <- list()
    if (length(ubis.treat.IDs) > 0) { 
...
2. Retain **`putative`** genes based on the `treat()` analysis for further clustering, networking and functional analysis. Only CTF normalised data will be considered as [previoulsy explained](#normBkg).
    ```{r putativeDEGs}
    ...
    rm(keep.putative)
    cat("Number of putative DEGs: ", nrow(x.filt.ctf.putat), "\n")
    ```


3. Correlations and distances should be calculated with **logarithmic counts** to group highly variant genes. However, this is configured in `configure_wf.R` in the variable `LOG_EXPR`. As explained above and in [@Johnson2022fn], only [CTF] transformation is affected.
...
4. Retain the **most variable genes** (rows) by filtering `putative` DEGs by [CV] as configured in `configure_wf.R`. When the `m` variables are in log-counts, CV will be much lower than CV for regular counts. Hence, configure `LOG_EXPR` to FALSE only when your experiment does not contain many variable genes.
    ```{r filterByCV}
...
5. A brief **summary** of the new sets of data for clustering:
...
***Ubiquitous DEGs***    |  | 
eBayes() genes    | `r ubis.eB`    | -
treat() genes     | `r ubis.treat` | -
