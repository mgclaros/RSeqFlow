...
OUTS_WARNING_MSG <- "**No outstanding** transcript detected. Be careful!!!"
if (is_OUTS_EMPTY) {
  most_outs <- max_outs
  warning("OUTS_WARNING_MSG")
}
```
...
if (is_OUTS_EMPTY) {
  warning("OUTS_WARNING_MSG", "\nHence, tThere is not **any outstanding** transcript to plot")
} else {
...
Let's show the gene profiles of outstanding genes in all conditions (left) and the most frequent outstanding IDs (right) provided that you have any outstanding transcript. 

`r if (is_OUTS_EMPTY) warning("OUTS_WARNING_MSG")`
...
`r if (!exists("MY_IDs")) warning("No user IDs were found in the configuration file")`
...
Saving the analysis that produced outstanding genes as well as the list of gene IDs for each condition, provided that you have any outstanding transcript to save. 

`r if (is_OUTS_EMPTY) warning("OUTS_WARNING_MSG", "\nNo OutstandingGenes- file was saved")`
