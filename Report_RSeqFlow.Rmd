...
date: "`r format(Sys.time(), '%A %d, %B %Y. %H.%M.%S')` | RMD version: 250113"
abstract: |
  `r paste(SOFT_NAME, VERSION_CODE)` [@Bullones2023aa] is a comprehensive R pipeline for processing RNA-Seq datasets from count files or tables in non-model organisms that perform the most recommended statistical analyses for differential expression and clustering. It carries out the following steps:
   ...
   4. Differential expression using `limma/voom` and `treat()` to seek for biological relevance [@Oshlack2010; @Marioni2008; @Li2022cf].
   5. Correlation analyses for clustering with 3 algorithms [@Usadel2009su; @Serin2016uw].
   6. Gene networks, including hubs and subgraphs [@Parsana2019jm; @Sia2022].
   7. Profiling outstanding genes.

   It can be cloned/dowloaded from its [GitHub repository](https://github.com/mgclaros/RSeqFlow). **You can USE AS IS without any warrants**.
